// from https://svn.apache.org/repos/asf/tika/trunk/tika-parsers/src/main/java/org/apache/tika/parser/microsoft/ExcelExtractor.java
/*
...
/**
 * Excel parser implementation which uses POI's Event API to handle the contents
 * of a Workbook.
 * <p>
 * The Event API uses a much smaller memory footprint than
 * <code>HSSFWorkbook</code> when processing excel files but at the cost of more
 * complexity.
 * <p>
 * With the Event API a <i>listener</i> is registered for specific record types
 * and those records are created, fired off to the listener and then discarded
 * as the stream is being processed.
 * 
 * @see org.apache.poi.hssf.eventusermodel.HSSFListener
 * @see <a href="http://poi.apache.org/hssf/how-to.html#event_api"> POI Event
 *      API How To</a>
 */
public class ExcelExtractor extends AbstractPOIFSExtractor {

	/**
	 * <code>true</code> if the HSSFListener should be registered to listen for
	 * all records or <code>false</code> (the default) if the listener should be
	 * configured to only receive specified records.
	 */
	...
	/**
	 * Returns <code>true</code> if this parser is configured to listen for all
	 * records instead of just the specified few.
	 */
	...
	/**
	 * Specifies whether this parser should to listen for all records or just
	 * for the specified few.
	 * <p>
	 * <strong>Note:</strong> Under normal operation this setting should be
	 * <code>false</code> (the default), but you can experiment with this
	 * setting for testing and debugging purposes.
	 * 
	 * @param listenForAllRecords
	 *            <code>true</code> if the HSSFListener should be registered to
	 *            listen for all records or <code>false</code> if the listener
	...
	/**
	 * Extracts text from an Excel Workbook writing the extracted content to the
	 * specified {@link Appendable}.
	 * 
	 * @param filesystem
	 *            POI file system
	 * @throws IOException
	 *             if an error occurs processing the workbook or writing the
	 *             extracted content
	 */
	protected void parse(final NPOIFSFileSystem filesystem,
			final XHTMLContentHandler xhtml, final Locale locale)
			throws IOException, SAXException, TikaException {
		final TikaHSSFListener listener = new TikaHSSFListener(xhtml, locale,
				this);
	...
	private static class TikaHSSFListener implements HSSFListener {
		 ...
		 * <code>null</code>, causes all subsequent HSSF records to be ignored
		 * and the stored exception to be thrown when
		...
		/**
		 * Extra text or cells that crops up, typically as part of a worksheet
		 * but not always.
		 */
		private List<Cell> extraTextCells = new ArrayList<Cell>();

		/**
		 * Format for rendering numbers in the worksheet. Currently we just use
		 * the platform default formatting.
		 * 
		 * @see <a
		...
		/**
		 * These aren't complete when we first see them, as the depend on
		 * continue records that aren't always contiguous. Collect them for
		 * later processing.
		 */
		private List<DrawingGroupRecord> drawingGroups = new ArrayList<DrawingGroupRecord>();

		/**
		 * Construct a new listener instance outputting parsed data to the
		 * specified XHTML content handler.
		 * 
		 * @param handler
		 *            Destination to write the parsed output to
		 */
		 ...
		 */
		public void processFile(NPOIFSFileSystem filesystem,
			...
			DocumentInputStream documentInputStream = filesystem
		...
		private void internalProcessRecord(Record record) throws SAXException,
			...
			case HyperlinkRecord.sid: // holds a URL associated with a cell
				if (currentSheet != null) {
					...
					Point point = new Point(link.getFirstColumn(),
							link.getFirstRow());
		...
		/**
		 * Adds a text cell with the given text comment. The given text is
		 * trimmed, and ignored if <code>null</code> or empty.
		 * 
		 * @param record
			...
			for (EscherRecord escherRecord : records) {
				if (escherRecord instanceof EscherBSERecord) {
					EscherBlipRecord blip = ((EscherBSERecord) escherRecord)
							.getBlipRecord();
					if (blip != null) {
						// TODO When we have upgraded POI, we can use this code
						// instead
						...
						TikaInputStream stream = TikaInputStream.get(blip
								.getPicturedata());

						// Handle the embeded resource
						extractor.handleEmbeddedResource(stream, null,
								mimeType, handler, true);
					}
				}
	...
	static {
		LOG = LoggerFactory.getLogger(ExcelExtractor.class);
	}
