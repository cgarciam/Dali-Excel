...
public class ElementaryCyclesSearch {
	 ...
	 */
	private boolean findCycles(final Integer v, final Integer s,
			Vector[] adjList) {
		boolean f = false;
		...
		if (f) {
			...
			for (int i = 0; i < adjList[v].size(); i++) {
				final Integer w = ((Integer) adjList[v].get(i)).intValue();
